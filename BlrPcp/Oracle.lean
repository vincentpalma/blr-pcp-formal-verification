...
@[blueprint
  (statement := /-- A \textit{PCP prover} is a function that takes an input $x$ and produces a proof $\pi : [n] → \F$,
  which is a function that maps queries to answers. $n$ is the \textit{proof length}. -/)]
abbrev PCPProver (α : Type) (F : Type) (n : ℕ) : Type :=
  α → (Fin n → F)

@[blueprint
  (statement := /-- A \textit{PCP verifier} is an oracle machine that takes an input $x$ and has access to two oracles:
  a randomness oracle that provides random bits, and a proof oracle that answers queries about the proof. -/)]
...
@[blueprint
  ...
  if there exists a prover $P$, a verifier $V$ and a polynomial $t$ such that for every $x ∈ L$,
  $P$ produces a proof $\pi$ of length $\ell(|x|)$,
  $V$ runs in time at most $t(|x|)$,
  makes at most $q(|x|)$ queries to the proof oracle
  and at most $r(|x|)$ queries to the randomness oracle, and satisfies the following:
  \begin{itemize}
    \item Completeness: If $x ∈ L$, then $\forall x \in L \quad \Pr\left[V^\pi(x)=1 \mid \pi \leftarrow P(x)\right] \geq 1 - \varepsilon_c$.
    \item Soundness: If $x \notin L$, then $\forall x \notin L \quad \forall \widetilde{P} \quad \Pr\left[V^{\widetilde{\pi}}(x)=1 \mid \widetilde{\pi} \leftarrow \widetilde{P}\right] \leq \varepsilon_s$.
  \end{itemize}
   -/)]
...
@[blueprint
  (statement := /-- $\mathrm{QESAT}(\F_2)$ is in $\mathrm{PCP}[ε_c = 0, ε_s = 1/2, Σ = \F_2, ℓ = \exp(n), q = O(1), r = n^{O(1)}]$. -/)]
